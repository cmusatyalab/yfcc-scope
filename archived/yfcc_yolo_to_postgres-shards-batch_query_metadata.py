...
BASE_URL = "https://storage.cmusatyalab.org/yfcc100m/yfcc100m-{000000..000402}.tar"
START_SHARD = 0  # Change this to resume from a specific tar file
MAX_SHARDS = 403  # Total number of tar files (000000 to 000402)
BATCH_SIZE = 1000  # Commit every N images

# Create webdataset pipeline
dataset = wds.WebDataset(BASE_URL, shardshuffle=False,handler=wds.warn_and_continue,)
...
batch_samples = []  # NEW: holds raw samples until we hit BATCH_SIZE
...
for sample in dataset:
    ...
    for s in batch_samples:
        try:
            ...
            # skip samples missing json etc.
            continue

    # -------------------------
    # NEW: one metadata query for this batch
    # -------------------------
    meta_dict = fetch_metadata_map(cur, id_to_sample.keys())

    # -------------------------
    # Process batch (mostly your original logic)
    # -------------------------
    for image_file_id, s in id_to_sample.items():
        try:
            # print(".")
            # Extract data from webdataset sample
            key = s["__key__"]  # e.g., "12345_abcdef1234"
            
            meta_row = meta_dict.get(image_file_id)
            if not meta_row:
                continue  # no metadata in DB → skip
            
            path = make_image_url_from_row(meta_row)
            # print(sample)
            jpg_bytes = sample["jpg"]  # image bytes
            # metadata = sample.get("json", {})  # optional metadata
    ...
    print(f"✅ Processed {processed} images | {imgs_per_sec:.2f} img/s")
...
# Final flush for leftover samples (same idea, minimal)
if batch_samples:
    id_to_sample = {}
    for s in batch_samples:
        try:
    ...
    for image_file_id, s in id_to_sample.items():
        ...
        try:
            ...
            for i, box in enumerate(bboxes, start=1):
                cx, cy, w, h = box["bbox_xywh"]
                bbox_buffer.append((image_file_id, i, box["label"], box["score"], cx, cy, w, h))
