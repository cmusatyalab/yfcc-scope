...
# -----------------------------------------------------------------------------
# Image-level bbox-count vectors (DB materialized table)
# -----------------------------------------------------------------------------
# Schema:
#   yfcc_image_label_counts(
#     image_file_id TEXT PRIMARY KEY,
#     total_bboxes INT NOT NULL,
#     counts JSONB NOT NULL,          -- {"person": 4, "car": 1, ...}
#     updated_at INT NOT NULL
#   )
...
def rebuild_vector_table():
    """
    Fully rebuild yfcc_image_label_counts from bb_table.
    counts = jsonb object of label->count for that image.
    total_bboxes = total # of rows in bb_table per image.
    """
    conn = open_conn()
...
# -----------------------------------------------------------------------------
# PCA Vector API: serve per-image bbox-count vectors
# -----------------------------------------------------------------------------
